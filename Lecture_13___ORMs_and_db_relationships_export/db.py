...
engine = create_engine('sqlite:///coll_uni_student.sqlite', echo=False) # You might also have a config file, for example, that holds the string that is the db name. For now we'll just put it here.
...
def init_db():
    ...
    return engine # Returning the engine makes it possible to use this function in other files (e.g. query files) to access the engine and use it
